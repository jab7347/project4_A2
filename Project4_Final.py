...
#Flask Setup to remote stream video
app = Flask(__name__)
                ...
                if len(result) > 0:  # If there was a face found
                    for face in result:
                        ...
                        if pName == reqPer:
                            ...
                            frame = cv2.circle(frame,( int(x+(w/2)), int(y + (h/2)) ),5,(0,255,0),2) #Center Dot for selected face
                            frame = cv2.circle(frame,(int(camCenterX),int(camCenterY)),5,(0,255,0),2) #Center dot for camera
                            frame = cv2.line(frame, ( int(x+(w/2)), int(y + (h/2)) ),(int(camCenterX),int(camCenterY)),(0,0,255),4) #Draws the line between the two points
                            frame = cv2.putText(frame,pName,(x,y),cv2.FONT_HERSHEY_SIMPLEX,1,(255,0,0),2) #Adds text for the detected person
                            faceFound = True
                            break
                        # end if
                    if faceFound:
                        ...
                        print(tx, ty)
                        if abs(tx) > PIXEL_TOLERANCE or abs(ty) > PIXEL_TOLERANCE: #Pixel Tolerance
                            rot = rot - tx #Calcuates new rotation value
                            el = el - ty #Calcultes new elevation value
                            if rot >= x_high_lim: #If outside bound
                                rot = x_high_lim
                            elif rot <= x_low_lim: #If outside bound
                                rot = x_low_lim
                            #End if
                            ret = arm.move_xyz(1, rot, el) #If moves arm
                            nextState = "NONE"
                        #End if
                    #End if
                #end if
                timeOutFrames = 4 #Time out frames
                outFrame = frame #Writes frame
            #End Case
        #End Select
        if timeOutFrames <=0: #Time out frames to redetect
            nextState = "FACE_DETECT"
        else:
            timeOutFrames -= 1
        #End If
